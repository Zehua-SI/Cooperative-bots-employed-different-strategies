...
// Global variables
int neighbors[N][neig_num]; // Store neighbors for each player
...
CategoryData robot, common_sv_09, common_sv_0;
int total_bots = 0, total_common_sv_09 = 0, total_common_sv_0 = 0;
...
void init_game(double r, double rho, double theta)
{
    ...
    for (int i = 0; i < N; i++) {
        if (type[i] == 1) sv[i] = theta; // if player is bot, its cooperation probability is theta
        else sv[i] = randf(); // if player is common, its cooperation probability is random
    }
}
...
double cal_payoff(int x) {
    ...
    for (int i = 0; i < neig_num; i++) {
        ...
        pay += (-r) * sv[x] + (1 + r) * sv[neig]; // Calculating accumulated pay-offs
    }
...
int main(void) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (type[i] == 1) {
                    ...
                    if (abs(sv[i] - 0.9) < 1e-5) {
                        common_sv_09.collect(pay);
                        total_common_sv_09++;
                    }
                    else if (abs(sv[i] - 0.0) < 1e-1){
                        common_sv_0.collect(pay);
                        total_common_sv_0++;
                    } 
                }
            }
    ...
    ofstream fout("Figure5_Continuous_pdf+cdf.csv");
    fout << "Category";
    ...
    print_freqs("Bot", robot.get_frequencies(total_bots));
    print_freqs("Common sv=0.9", common_sv_09.get_frequencies(total_common_sv_09));
    print_freqs("Common sv=0.0", common_sv_0.get_frequencies(total_common_sv_0));
