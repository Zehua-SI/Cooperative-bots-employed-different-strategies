...
double sv[N];                             // Strategy values for player
int type[N];                              // Type of player (0 for common, 1 for robot)
...
CategoryData robot, common_coop, common_defect;
int total_bots = 0, total_commons = 0, total_commons_coop = 0, total_commons_defect = 0;
...
void init_game(double r, double rho, double theta)
{
    ...
    // Initialize the strategy for the ordinary players and bots
    for (int i = 0; i < N; i++) {
        if (type[i] == 1) 
        {   
            sv[i] = theta; 
            if (sv[i] >= randf()) strategy[i] = 0;
            else strategy[i] = 1; // compare the cooperation possibility (strategy value) with random number, choose cooperation(0) or defection(1)
            // if player is bot, its cooperation possibility (strategy value) is theta
        }
        else {
            strategy[i] = randi(2); 
        } 
...
// Calculate payoff for an player
...
// Update strategy for an player based on the differences in neighbor's payoff and focal player's payoff
...
int main(void) {
        ...
        if (step >= TOTAL_STEPS - COLLECT_STEPS) {
            for (int i = 0; i < N; i++) {
                ...
                if (type[i] == 1) {
                    ...
                    total_commons++;
                    if (strategy[i] == 0) {
                        common_coop.collect(pay);
                        total_commons_coop++;
                    }
                    else {
                        common_defect.collect(pay);
                        total_commons_defect++;
                    } 
                }
            }
        }
    ...
    ofstream fout("Figure5_Discrete_pdf+cdf.csv");
    fout << "Category";
    ...
    print_freqs("Robot", robot.get_frequencies(total_bots));
    print_freqs("Common Coop", common_coop.get_frequencies(total_commons_coop));
    print_freqs("Common Defect", common_defect.get_frequencies(total_commons_defect));
