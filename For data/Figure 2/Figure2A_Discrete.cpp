...
double m;                // Exponent for Kappa calculation
...
int main(void) 
{
    ...
    FILE *FC = fopen("Figure2A_Discrete.csv", "w");  // Open file for writing results
    ...
    for(m = -6; m <= 4 ; m += 1){  // Loop over dilemma strength
        for(rho = 0.00; rho <= 0.51; rho += 0.01) {  // Loop over cimitation strength
