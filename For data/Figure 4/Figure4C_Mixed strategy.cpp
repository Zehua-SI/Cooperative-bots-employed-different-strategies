...
double Kappa;    // Imitation strength (controls how likely players are to imitate their neighbors)
double m;        // power of exponent
double r = 0.1;              // Dilemma strength (controls the temptation to defect)
...
void init_game(double r, double rho, double m)
{
    ...
    // Initialize strategy values and strategies for bots and ordinary players
    for (int i = 0; i < N; i++) {
        if (type[i] == 1) sv[i] = theta; // the strategy value of bots/ the probability of choosing cooperation for bots is theta
        else sv[i] = randf(); // the strategy value of ordinary player/ the probability of choosing cooperation for ordinary player is theta
        
        // Determine initial action based on strategy value
        if (sv[i] >= randf()) strategy[i] = 0;  // Cooperate
        else strategy[i] = 1;  // Defect
    }
}
...
double cal_payoff(int x) {
    ...
    for (int i = 0; i < neig_num; i++) {
        // Focal player chooses action based on strategy value
        if (sv[x] <= randf()) strategy[x] = 1; // Defect
        else strategy[x] = 0; // Cooperate
        
        neig = neighbors[x][i]; // Find neighbor
        // Neighbor chooses action based on strategy value
        if (sv[neig] <= randf()) strategy[neig] = 1; // Defect
        else strategy[neig] = 0; // Cooperate
        
        pay += payoff_matrix[strategy[x]][strategy[neig]]; // Calculate payoff
    }
    return pay;
}
...
void cal_data() {
    ...
    for (int i = 0; i < N; i++) {
        if (type[i] == 0) {  // Only consider ordinary players
            nn++;
            x += sv[i];        // the probability of choosing cooperation
            y += (1 - sv[i]);  // the probability of choosing defection
        }
    }
...
// Calculate mean and standard deviation of probabilty of choosing cooperation and defection
void cal_dev(void) {
    // Calculate means
    mean_temp_c = 0;
    mean_temp_d = 0;
...
int main(void) {
    ...
    FILE *Fc = fopen("Figure4C_Mixed strategy.csv", "w");  // Open file for writing results
    
    // Main simulation loop
    for (m = -6; m < 4; m += 1) {  // Loop over dilemma strength
        for (rho = 0; rho < 0.51; rho += 0.01) {  // Loop over bot proportion
            for (int lo = 0; lo < loop; lo++) {  // Repeat each parameter set 100 times
                ...
                printf("%f\t%f\t%f\t%f\n", Kappa, rho, c_temp/2000, d_temp/2000);
            }
