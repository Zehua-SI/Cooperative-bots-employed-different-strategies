...
double r = 0.01;                    // Dilemma strength (controls the temptation to defect)
...
int main(void) 
{
    ...
    FILE *FC = fopen("Figure6_Discrete_r=0.01.csv", "w");  // Open file for writing results
    ...
    for(rho = 0.00; rho <= 0.51 ; rho += 0.01){  // Loop over the proportion of bots
        for(theta = 0.0; theta <= 1.01; theta += 0.01) {  // Loop over strategy value of bots
