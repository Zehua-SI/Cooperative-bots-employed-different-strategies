...
int main(void) {
    ...
    FILE *Fc = fopen("Figure3C_Mixed strategy.csv", "w");  // Open file for writing results
    
    // Main simulation loop
    for (r = 0; r < 0.21; r += 0.01) {  // Loop over dilemma strength
        for (rho = 0; rho < 0.51; rho += 0.01) {  // Loop over bot proportion
            for (int lo = 0; lo < loop; lo++) {  // Repeat each parameter set 100 times
                ...
                printf("%f\t%f\t%f\t%f\n", r, rho, c_temp/2000, d_temp/2000);
            }
            
            cal_dev();  // Calculate mean and standard deviation
            // Write results to file
		fprintf(Fc,"%f,%f,%f,%f,%f,%f\n",r,rho,mean_temp_c,mean_temp_d,dev_temp_c,dev_temp_d);
