...
double Kappa = 0.1;    // Imitation strength
double r;              // cdilemma strength
...
int main(void) {
    ...
    FILE *Fc = fopen("Figure3A_Discrete strategy.csv", "w");
    
    // Main simulation loop
    for (r = 0.00; r < 0.21; r += 0.01) {
    	for(rho = 0.00; rho < 0.51; rho += 0.01){
	        for(int lo = 0; lo < loop; lo++) {
	            ...
	            printf("%f\t%f\t%f\t%f\n", r, rho, (double)c_temp/2000, (double)d_temp/2000);
	        }
